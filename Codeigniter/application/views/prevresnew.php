...
<html lang="en">
  ...
  <?php echo '<h1>Welcome '.$_SESSION['use'].'</h1>';?>
    ...
    <button type="submit" formaction="<?php echo base_url(); ?>index.php/loginprocess/index_title_prev" class="btn btn-primary"  name="sbt">Search by Title</button>
    <button type="submit" formaction="<?php echo base_url(); ?>index.php/loginprocess/index_author_prev" class="btn btn-success" class="author" name="sba">Search by Author</button>
    </form>
    <form method="post" action="<?php echo base_url(); ?>index.php/loginprocess/session_prev">
 ...
 <div class="container">
  ...
  <div class="page-header">
  <?php echo "<h1><strong>Displaying ".$num_rows." results for your search:</strong></h1>"; ?> 
  </div>
   <div class="row">
  ...
  <img src="<?php echo base_url('images/'.$titles['ISBN'].'.jpeg') ?>">;
  </div>
  <form method="post" action="<?php echo base_url(); ?>index.php/cart/processcart/<?php echo $titles['ISBN']?>">
  ...
  <br>
  <form action="<?php echo base_url(); ?>index.php/cart/gotocart?>">
  ...
  <div class="thumbnail text-center" >
  <div class="img-thumbnail" text-align="center">
 <img src="<?php echo base_url('images/'.$authors['ISBN'].'.jpeg') ?>">;
  </div>
  ...
  <p style="color:red">Copies:<?php echo $authors['Number']; ?></p>
   <form method="post" action="<?php echo base_url(); ?>index.php/cart/processcart/<?php echo $authors['ISBN']?>">
  Select Quantity:<input type="number" name="quantity" min=1 max=25><br>
...
<div class="container">
  ...
  <?php echo "<h1><strong>Displaying ".$num_rows." results for your search:</strong></h1>"; ?> 
   ...
   <div class="row">
  <?php foreach($prev as $previous){
  ?>
  <div class="col-md-3">
  <div class="thumbnail text-center" >
  <div class="img-thumbnail" text-align="center">
  <img src="<?php echo base_url('images/'.$previous['ISBN'].'.jpeg') ?>">;
  </div>
  <form method="post" action="<?php echo base_url(); ?>index.php/cart/processcart/<?php echo $previous['ISBN']?>">
  <p><strong><?php echo $previous['Title']; ?></strong></p> 
  <p><strong><?php echo $previous['ISBN']; ?></strong></p>
  <p style="color:red">Copies:<?php echo $previous['Number']; ?></p>
  Select Quantity:<input type="number" name="quantity" min=1 max=25><br>
  <input type="hidden" name="hiddentitle" value="<?php echo $previous['Title'] ?>">
  <input type="hidden" name="hiddencopy" value="<?php echo $previous['Number'] ?>">
  <input type="hidden" name="hiddenprice" value="<?php echo $previous['Price'] ?>">
  <br><p><span><button type="submit" class="btn btn-success" name="cart"><span class="glyphicon glyphicon-shopping-cart"></span><?php echo $previous['name'].': '?>Add to Cart</button></span></p>
  <input type="hidden" name="hiddenwarecode" value="<?php echo $previous['warehouseCode'] ?>">
  </form>
  </div>
  </div>
  <?php }?>
  ...
  <?php } ?>
    ...
    <script src="<?php echo base_url("bootstrap/css/js/bootstrap.min.js"); ?>"></script>
  </body>
</html>